...
```sh
npm install git+ssh://git@github.com:stainless-sdks/premai-typescript.git
```

> [!NOTE]
> Once this package is [published to npm](https://app.stainless.com/docs/guides/publish), this will become: `npm install premai`
...
async function main() {
  const response = await client.chat.retrieveInternalModels();
}
...
async function main() {
  const response: unknown = await client.chat.retrieveInternalModels();
}
...
await client.chat.retrieveInternalModels({
  maxRetries: 5,
});
```
...
const { data: response, response: raw } = await client.chat.retrieveInternalModels().withResponse();
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/premai-typescript/issues) with questions, bugs, or suggestions.
